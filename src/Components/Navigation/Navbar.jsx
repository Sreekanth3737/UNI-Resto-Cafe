...
const Navbar = () => {
  ...
  return (
    <nav>
    <div className=' w-full flex bg-white justify-between h-13 pt-3'>
        {menuList?.map((item,index)=>{
            return(
                <div key={index}>
                <h1 className='my-3 font-medium mx-3 text-zinc-600'> {  item?.restaurant_name}</h1>
                </div>
            )
        })}
      ...
      <span className='bg-red-600 text-white rounded-full h-6 text-center w-6 mr-5'>{cartCount}</span>
      </div>
